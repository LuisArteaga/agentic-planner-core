    ...
    if os.path.isfile(context_file):
        try:
            with open(context_file, "r", encoding="utf-8", errors="replace") as f:
                context_lines.append(f"--- {os.path.relpath(context_file, workspace_dir)} ---")
                context_lines.append(f.read())
                context_lines.append("")
        except Exception as e:
            sys.stdout.write(f"[WARN] Failed to read {context_file}: {e}\n")
    else:
        sys.stdout.write(f"[WARN] Architecture context file (CONTEXT.md or docs/context.md) is missing.\n")
    ...
    if os.path.isdir(adr_dir):
        ...
        if os.path.isdir(flat_adr_dir):
            try:
                ...
                sys.stdout.write(f"[WARN] Failed to read ADR files from {flat_adr_dir}: {e}\n")
        else:
            sys.stdout.write(f"[WARN] Architecture Decision Records folder (docs/adr or adr) is missing.\n")
